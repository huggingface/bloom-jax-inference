...
def server_loop(q):
    ...
    try:
        while True:
            ...
            print("Len queus", len(all_queues))
...
threading.Thread(target=server_loop, args=(q, )).start()

app = Flask(__name__)

@app.route("/generate", methods=["POST"])
def generate():
    print(f"Got new query")
    content = request.json

    if not content["parameters"]["do_sample"]:
        return make_response({"error": "Not accepting sampling atm."}, 500)
    ...
    response =  make_response(jsonify([{"generated_text": out}]), 200)
    print(f"Replying with response {response}")
    return response
