...
def copy_files_on_tpu(path, dest, is_dir=False):
    if is_dir:
        command = f"gcloud compute tpus tpu-vm scp --recurse {path} suraj-tpu-v3-32:{dest} --quiet --force-key-file-overwrite --strict-host-key-checking=no --worker=all --zone=europe-west4-a"
    else:
...
def run_command_on_tpu(command):
    command = f"gcloud compute tpus tpu-vm ssh suraj-tpu-v3-32 --zone=europe-west4-a --quiet --force-key-file-overwrite --strict-host-key-checking=no --worker=0 --command='{command}'"
    return subprocess.check_output(command, shell=True).decode("utf-8").strip()
...
def start_ray(conn, address):
    ...
    # run_command_on_tpu(f"sudo /tmp/ray-tpu.sh {address}")